...
from perm import find_permutations
...
class Combination:
    ...
    def __init__(self, combination: List[int]) -> None:
        """Construct a Combination object while assigning the combination list."""
...
def find_combinations(num_of_cards: int) -> List[Combination]:
    ...
    combinations: List[Combination] = [
        Combination(list(x)) for x in find_permutations(elements, num_of_cards, False)
    ]
    print(
    ...
    with tqdm(total=iterations) as progress:
        for count, perm in enumerate(combinations):
            ...
            for _ in range(cycles):
                shuffle(card_deck)
                perm_total_sum += min(
                    x.get_pos_of_last_number(card_deck) for x in perm.get_cards()
                )
                progress.update()
...
def print_results(combinations: List[Combination], terse: int = 0) -> None:
    ...
    for i, comb in enumerate(combinations):
        if terse == 0 or i < terse or i >= len(combinations) - terse:
            print(
                f"{i+1:11}: {' '.join(f'{x.draw_terminal_color()}' for x in comb.get_cards())} "
